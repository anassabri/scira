...
export let dodoPayments: DodoPayments | null = null;
...
if (serverEnv.DODO_PAYMENTS_API_KEY) {
  ...
  const dodoPaymentsPlugin = dodopayments({
    client: dodoPayments,
    createCustomerOnSignUp: true,
      use: [
        dodocheckout({
  ...
  plugins.unshift(dodoPaymentsPlugin);
}
